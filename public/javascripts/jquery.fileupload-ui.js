...
(function ($) {
    ...
    $.widget('blueimpUI.fileupload', $.blueimp.fileupload, {
        ...
        options: {
            ...
            minImageWidth: 600,
            // Minimum acceptable image height
            minImageHeight: 400,
            
            // Private variable to hold number of NEW items added to the queue.
            ...
            add: function (e, data) {
            	...
            	// Custom AJAX call to get fabric limit from server
            	jQuery.ajax({
            		url: "get-upload-info",
            		async: false,
            		dataType: 'json',
            		success: function(jsondata) {
						var uploaded_count = jsondata.uploaded_count;
						var limit = jsondata.limit;
						that.options.maxNumberOfFiles = (limit >= (uploaded_count + that.options.numberOfNewItems)) ? (limit - (uploaded_count + that.options.numberOfNewItems)) : 0;
            		}
        		});
                ...
                that.options.numberOfNewItems += 1;
        ...
        _loadImage: function (file, callback, callback2, options) {
            var that = this,
                url,
                img;
            if (!options || !options.fileTypes ||
                ...
                img = $('<img>').bind('load', function () {
                    ...
                    // before they are modified by _scaleImage
                    var img_dim = new Array();
                    img_dim.width = img[0].width;
                    img_dim.height = img[0].height;
                    var img_object = that._scaleImage(img[0], options);
                    
                    callback(img_object);
                    callback2(img_object, img_dim);
                });
                if (url) {
                    img.prop('src', url);
                } else {
        ...
        _renderUpload: function (files) {
            ...
            tmpl.find('.preview').each(function (index, node) {
                that._loadImage(
                    ...
                    function(img_object, img_dim) {
                    	// We can use this function to enable 
                    	// the error message right here.
                    	if ( img_dim.width < options.minImageWidth || 
                    		img_dim.height < options.minImageHeight )
                		{
	                    	var temp_upload = $(img_object).closest('.template-upload');
	                    	// Make sure an error has not already been set.
	                    	if(!temp_upload.hasClass('ui-state-error'))
	                    	{
	                    		// Add the error class
	                    		temp_upload.addClass('ui-state-error');
	                    		// Remove progress and start columns
	                    		temp_upload.find('.progress').remove();
	                    		temp_upload.find('.start').remove();
	                    		// Add error column
	                    		var cancel_col = temp_upload.find('.cancel');
	                    		$("<td class='error' colspan='2'>The image is too small. Please make sure the image is at least 600x400 pixels.</td>").insertBefore(cancel_col);
	                    	}
	                    }
                    },
                    {
